...
<script type="text/javascript">
	...
	function validation(teamId,teamMemberId) {
		...
		if (!regexp.test(gName)) { dataTypeMessage += "<br>Name can contain [alphabets,.,-] min 3, max 20."; } 
		...
		if (lDate > currentDate) { mustSelectMessage += "<br>Leave date can't be in future."; jQuery("#leaveTip").hide(); document.getElementById("leaveDate").value = ""; } 
		if (document.getElementById("voided").checked && (reason == null || reason == "")) { mustSelectMessage += "<br>Either write a reason or uncheck the box please."; } 
			...
			xmlhttp.onreadystatechange=function() {
				if(xmlhttp.readyState==4 && xmlhttp.status==200) {
					x=xmlhttp.responseText;
			   		if(x == "" || x == null){ document.getElementById("editMember").submit(); }
...
<form:form id="editMember" name="editMember" commandName="memberData"> <!-- method="post"  -->
	<table class="team">
		...
		<tr id="memberJoin">
			...
			<td>
				<form:input id="joinDate" path="joinDate" onfocus="showCalendar(this,60);jQuery('#joinTip').show();join();" onblur="jQuery('#joinTip).hide()" />
				<span id="joinTip">
		...
		<tr>
			...
			<td>
			<form:textarea id="voidReason" path="voidReason" /></td>
		</tr>
		<tr>
			<td></td>
			<td>
				<button id="useExistingButton" type="button" onClick="validation(${teamId},${teamMemberId});">
					Edit
				</button>
			</td>
		</tr>
